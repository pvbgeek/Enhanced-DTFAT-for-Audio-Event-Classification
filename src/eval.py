...
def validate_best(audio_model, val_loader, args, epoch):
    """
    Runs evaluation on the best saved model checkpoint.
    """
...
if __name__ == '__main__':
    ...
    val_ds = dataloader.AudiosetDataset(
        args.data_val,
    ...
    # ---- Build model — use correct class based on variant ----
    print(f"Building model for variant: {args.model_variant}")
    if args.model_variant == 'baseline':
        from the_new_audio_model import get_timm_pretrained_model
        audio_model = get_timm_pretrained_model(
            num_classes_dict[args.dataset],
            imgnet=args.imagenet_pretrain
        )
    else:
        from the_new_audio_model_enhanced import get_enhanced_model
        audio_model = get_enhanced_model(
            num_classes_dict[args.dataset],
            imgnet=args.imagenet_pretrain,
    ...
    for k, v in wt.items():
        ...
        new_state[new_k] = v
    audio_model.load_state_dict(new_state, strict=True)
    print('\nNow starting validation with best model...')
